...
class BookOrder extends React.Component {
  ...
  componentDidMount = () => {
    ...
    const action = axios.get("/iron/read");
    action
      .then((response) => {
    ...
    const dryClean = axios.get("/dryClean/read");
    dryClean
      .then((response) => {
    ...
    const wash = axios.get("/wash/read");
    wash
      .then((response) => {
        console.log(response.data);
        this.setState({
    ...
    switch (action) {
      case "Wash":
        ...
        if (stateCopy.washCloth[index].cloth_quantity > 1)
          stateCopy.washCloth[index].cloth_quantity--;
        else alert("You have To select one item for order");
        this.setState({
          washCloth: stateCopy.washCloth,
        });
        break;
    ...
    return (
      <div>
        <div className="makeOrder col-lg-12 col-sm-12 col-md-12 col  mx-auto border border-dark rounded p-2 mt-3">
          <h1 className="text-center border rounded p-2 m-2">
            Make Your Order
          </h1>
          <div className="flexWrapper selectbox  border rounded mt-3 pt-3">
            ...
            <div className="form-group">
              <select
                ...
                className="form-control"
                onChange={(event) => {
                  this.handleTime(event.target.value);
                }}
              >
                <option defaultValue>Select Time Slot</option>
                {this.props.timeSlot &&
                  this.props.timeSlot.map((time, index) => {
                    return (
            ...
            <div className="form-group">
              <select
                ...
                className="form-control"
                onChange={(event) => {
                  this.handleAction(event.target.value);
                }}
              >
                <option defaultValue>Select Wash Type</option>
                {this.props.washAction &&
                  this.props.washAction.map((action, index) => {
                    return (
          ...
          <div className="row clothlist mt-2">
            {this.props.userSelectedActionType === "Wash" &&
              this.state.washCloth.map((cloth, index) => {
                return (
                  <div
                    className="cloth_item  col col-sm-2 border m-2 mx-auto rounded shadow p-2"
                    key={index}
                  >
                    <Card key={index}>
                      <Card.Content>
            ...
            {this.props.userSelectedActionType === "Iron" &&
              this.state.ironCloth.map((cloth, index) => {
                return (
                  <div className="">
                    <div
                      className="cloth_item border m-2 rounded shadow p-2"
                      key={index}
                    >
                      <Card.Group>
                        <Card>
          ...
          {this.props.userSelectedActionType && (
            <div>
              <button
                className="fetchButton mt-2"
                onClick={(event) => {
                  this.handleOrder(event.target);
                }}
              >
                Book Order
              </button>
