...
const orderReducer = (state = userOrderState, action) => {
  ...
  switch (action.type) {
    ...
    case "book_order":
      ...
      let result = axios.post("/order/add", order);
      result
        .then((response) => {
          ...
          result = axios.post("/cloth/add", stateCopy.items);
          result
            .then((response) => {
    ...
    case "removeQuantity":
      ...
      if (stateCopy.items[itemindex].cloth_quantity > 1) {
        ...
        totalvalue =
          stateCopy.items[itemindex].total - stateCopy.items[itemindex].price;

        stateCopy.items[itemindex].total = totalvalue;
        stateCopy.totalCost =
          stateCopy.totalCost - stateCopy.items[itemindex].price;
      } else alert("Select 1 Piece For Order");
    ...
    case "removeItem":
      let itemIndex = action.payload;
      totalvalue =
        stateCopy.items[itemIndex].price *
        stateCopy.items[itemIndex].cloth_quantity;
